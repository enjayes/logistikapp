    ...
    antwortNachricht: {
        ...
        handleMessage: function (clientController, client, data) {
            ...
            if (data.t == this.messageType.getAll) {
                ...
                dataController.mysqlConnection.query('SELECT * FROM Jobs WHERE lieferanten_id = "'+ data.lid+'"' , function (err, rows, fields) {
                    if (err)
                        console.log("MYSQL ERROR: " + err);
                    else {
                        if (rows) {
                            var jobs = rows;
                            dataController.mysqlConnection.query('SELECT TOP 1 * FROM Jobs AND lieferanten_id = "'+ data.lid +'" ORDER BY timestamp_start DESC', function (err, rows, fields) {
                                if (err)
                                    console.log("MYSQL ERROR: " + err);
                                else {
                                    if (rows) {
