                    ...
                    if (rows && rows.length > 0) {
                        ...
                        for (var i = 0; i < rows.length; i++) {
                            ...
                            var nachricht = {
                                ...
                                nachricht: "Alarm: Der Termin \"" + termin.Title + "\" wurde nicht wahrgenommen!"
    ...
    termin: {
        ...
        handleMessage: function (clientController, client, data) {
            ...
            if (data.t == this.messageType.getRange) {
                ...
                if (data.lid) {
                    var sqlQuery = 'SELECT * FROM Termine WHERE ((RepeatDays = 0 AND StartMilli >= ? AND ((EndMilli <= ? AND EndMilli <> 0) OR (StartMilli <= ? AND EndMilli = 0 ))) OR (RepeatDays > 0))  AND Lieferant = ?';
                    var sqlVars = [start, end, end, data.lid];
                }
                else {
                    sqlQuery = 'SELECT * FROM Termine WHERE (RepeatDays = 0 AND StartMilli >= ? AND ((EndMilli <= ? AND EndMilli <> 0) OR (StartMilli <= ? AND EndMilli = 0 ))) OR (RepeatDays > 0)';
                                ...
                                if (event.RepeatDays > 0) {
                                    ...
                                    for (var day = start; day <= end; day = day + 86400000) {
                                        ...
                                        if (startDiff >= 0 && startDiff < 86400000) {
                                            ...
                                            if (repeatEvent.StartingMilli < repeatEvent.StartMilli) //today < repeatEvent.StartMilli
                ...
                dataController.termin.getLieferantenTermine(data.lid, function (termine) {
                    client.socket.emit('termin', {t: "cb", callback: data.callback, cbdata: termine});
    ...
    phone: {
        ...
        handleMessage: function (clientController, client, data) {
            ...
            if (data.t == this.messageType.getAll) {
                ...
                if (job) {
                    if (job.terminId) {
                        var terminId = job.terminId;
                        var terminStartMilli = job.terminStartMilli;
                        var terminEndMilli = job.terminEndMilli;
                        delete  job.terminId;
                        delete  job.terminStartMilli;
                        delete  job.terminEndMilli;
                    ...
                    if (terminId) {
                        ...
                        dataController.mysqlConnection.query('SELECT * FROM Termine WHERE id = ? ', terminId, function (err, rows, fields) {
                            if (err)
                                console.log("MYSQL ERROR: " + err);
                            else {
