...
/**
 * The Class ExpandedResWindow.
 */
public class ExpandedResWindow extends SearchWindow {

	/**
	 * Instantiates a new expanded res window.
	 *
	 * @param tsI the ts i
	 * @param tsJ the ts j
	 * @param shrunkI the shrunk i
	 * @param shrunkJ the shrunk j
	 * @param shrunkWarpPath the shrunk warp path
	 * @param searchRadius the search radius
	 */
	public ExpandedResWindow(TimeSeries tsI, TimeSeries tsJ, PAA shrunkI,
			PAA shrunkJ, WarpPath shrunkWarpPath, int searchRadius) {
		// Initialize the private data in the super class.
		super(tsI.size(), tsJ.size());

		// Variables to keep track of the current location of the higher
		// resolution projected path.
		int currentI = shrunkWarpPath.minI();
		int currentJ = shrunkWarpPath.minJ();

		// Variables to keep track of the last part of the low-resolution warp
		// path that was evaluated
		// (to determine direction).
		int lastWarpedI = Integer.MAX_VALUE;
		int lastWarpedJ = Integer.MAX_VALUE;

		// For each part of the low-resolution warp path, project that path to
		// the higher resolution by filling in the
		...
		for (int w = 0; w < shrunkWarpPath.size(); w++) {
			...
			// If the path moved up or diagonally, then the next cell's values
			// on the J axis will be larger.
			...
			// If a diagonal move was performed, add 2 cells to the edges of the
			// 2 blocks in the projected path to create
			...
			// Fill in the cells that are created by a projection from the cell
			// in the low-resolution warp path to a
			// higher resolution.
			...
			// Record the last position in the warp path so the direction of the
			// path can be determined when the next
