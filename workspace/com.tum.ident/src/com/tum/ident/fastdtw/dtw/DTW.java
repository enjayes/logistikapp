...
public class DTW {
	 ...
	 * @return the double
	 */
	public static double calcWarpCost(WarpPath path, TimeSeries tsI,
			TimeSeries tsJ, DistanceFunction distFn) {
		double totalCost = 0.0;

		for (int p = 0; p < path.size(); p++) {
			final com.tum.ident.fastdtw.matrix.ColMajorCell currWarp = path
					.get(p);
			totalCost += distFn.calcDistance(
					tsI.getMeasurementVector(currWarp.getCol()),
	...
	// require the entire cost matrix to be filled and only needs 2 columns to
	...
	public static double getWarpDistBetween(TimeSeries tsI, TimeSeries tsJ,
		...
		// The space complexity is 2*tsJ.size(). Dynamic time warping is
		// symmetric so switching the two time series
		// parameters does not effect the final warp cost but can reduce the
		// space complexity by allowing tsJ to
		// be set as the shorter time series and only requiring 2 columns of
		// size |tsJ| rather than 2 larger columns of
		// size |tsI|.
		...
		currCol[0] = distFn.calcDistance(tsI.getMeasurementVector(0),
				tsJ.getMeasurementVector(0)); // first cell
		...
		for (int i = 1; i <= maxI; i++) // i = columns
		{
			...
			{
				// (i,j) = LocalCost(i,j) +
				// minGlobalCost{(i-1,j),(i-1,j-1),(i,j-1)}
	...
	private static TimeWarpInfo DynamicTimeWarp(TimeSeries tsI, TimeSeries tsJ,
		...
		for (int i = 1; i <= maxI; i++) // i = columns
		{
			...
			for (int j = 1; j <= maxJ; j++) // j = rows
			{
				// (i,j) = LocalCost(i,j) +
				// minGlobalCost{(i-1,j),(i-1,j-1),(i,j-1)}
				final double minGlobalCost = Math
						.min(costMatrix[i - 1][j], Math.min(
		...
		/*
		 * // writes a section of the cost matrix to a file try { final
		 * PrintWriter out = new PrintWriter(new FileWriter("matrix1.csv")); for
		 * (int j=maxJ; j>=0; j--) { for (int i=0; i<=maxI; i++) {
		 * out.print(costMatrix[i][j]); if (i != maxI) out.print(","); }
		 * out.println(); } out.flush(); out.close(); } catch (Exception e) {
	...
	public static double getWarpDistBetween(TimeSeries tsI, TimeSeries tsJ,
		...
		while (matrixIterator.hasNext()) {
			final ColMajorCell currentCell = (ColMajorCell) matrixIterator
					.next(); // current cell being filled
			...
			{
				final double minGlobalCost = Math.min(
						costMatrix.get(i - 1, j),
	 ...
	 */
	public static TimeWarpInfo getWarpInfoBetween(TimeSeries tsI,
			TimeSeries tsJ, SearchWindow window, DistanceFunction distFn) {
		return constrainedTimeWarp(tsI, tsJ, window, distFn);
	}

	/**
	 * Constrained time warp.
	 *
	 * @param tsI the ts i
	 * @param tsJ the ts j
	 * @param window the window
	 ...
	 * @return the time warp info
	 */
	private static TimeWarpInfo constrainedTimeWarp(TimeSeries tsI,
			TimeSeries tsJ, SearchWindow window, DistanceFunction distFn) {
		// COST MATRIX:
		// 5|_|_|_|_|_|_|E| E = min Global Cost
		// 4|_|_|_|_|_|_|_| S = Start point
		 ...
		 * FileWriter("matrix2.csv")); for (int j=maxJ; j>=0; j--) { for (int
		 * i=0; i<=maxI; i++) { out.print(costMatrix.get(i, j)); if (i != maxI)
		 * out.print(","); } out.println(); } out.flush(); out.close(); } catch
