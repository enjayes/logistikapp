...
class RealDoubleFFT_Mixed {
	...
	void radb2(int ido, int l1, final double cc[], double ch[],
		...
		if (ido != 2) {
			for (k = 0; k < l1; ++k) {
				for (i = 2; i < ido; i += 2) {
					ic = ido - i;
					ch[i - 1 + k * ido] = cc[i - 1 + 2 * k * ido]
							+ cc[ic - 1 + (2 * k + 1) * ido];
					tr2 = cc[i - 1 + 2 * k * ido]
							- cc[ic - 1 + (2 * k + 1) * ido];
					ch[i + k * ido] = cc[i + 2 * k * ido]
							- cc[ic + (2 * k + 1) * ido];
					ti2 = cc[i + (2 * k) * ido] + cc[ic + (2 * k + 1) * ido];
					ch[i - 1 + (k + l1) * ido] = wtable[i - 2 + iw1] * tr2
							- wtable[i - 1 + iw1] * ti2;
					ch[i + (k + l1) * ido] = wtable[i - 2 + iw1] * ti2
							+ wtable[i - 1 + iw1] * tr2;
				}
			}
			if (ido % 2 == 1)
	...
	void radb3(int ido, int l1, final double cc[], double ch[],
		...
		for (k = 0; k < l1; k++) {
			for (i = 2; i < ido; i += 2) {
				...
				cr3 = taui
						* (cc[i - 1 + (3 * k + 2) * ido] - cc[ic - 1
								+ (3 * k + 1) * ido]);
				ci3 = taui
						* (cc[i + (3 * k + 2) * ido] + cc[ic + (3 * k + 1)
								* ido]);
				dr2 = cr2 - ci3;
				dr3 = cr2 + ci3;
				di2 = ci2 + cr3;
	...
	void radf4(int ido, int l1, final double cc[], double ch[],
		...
		for (k = 0; k < l1; k++) {
			ti1 = -hsqt2
					* (cc[ido - 1 + (k + l1) * ido] + cc[ido - 1 + (k + 3 * l1)
							* ido]);
			tr1 = hsqt2
					* (cc[ido - 1 + (k + l1) * ido] - cc[ido - 1 + (k + 3 * l1)
							* ido]);
		...
		if (ido != 2) {
			for (k = 0; k < l1; ++k) {
				for (i = 2; i < ido; i += 2) {
					...
					tr2 = cc[i - 1 + 4 * k * ido]
							+ cc[ic - 1 + (4 * k + 3) * ido];
					ti4 = cc[i - 1 + (4 * k + 2) * ido]
							- cc[ic - 1 + (4 * k + 1) * ido];
					tr3 = cc[i - 1 + (4 * k + 2) * ido]
							+ cc[ic - 1 + (4 * k + 1) * ido];
	...
	void radf5(int ido, int l1, final double cc[], double ch[],
		...
		for (k = 0; k < l1; k++) {
			...
			ch[ido - 1 + (5 * k + 1) * ido] = cc[k * ido] + tr11 * cr2 + tr12
					* cr3;
			ch[(5 * k + 2) * ido] = ti11 * ci5 + ti12 * ci4;
			ch[ido - 1 + (5 * k + 3) * ido] = cc[k * ido] + tr12 * cr2 + tr11
					* cr3;
	...
	void radb5(int ido, int l1, final double cc[], double ch[],
		...
		for (k = 0; k < l1; ++k) {
			for (i = 2; i < ido; i += 2) {
				...
				tr5 = cc[i - 1 + (5 * k + 2) * ido]
						- cc[ic - 1 + (5 * k + 1) * ido];
				tr2 = cc[i - 1 + (5 * k + 2) * ido]
						+ cc[ic - 1 + (5 * k + 1) * ido];
				tr4 = cc[i - 1 + (5 * k + 4) * ido]
						- cc[ic - 1 + (5 * k + 3) * ido];
				tr3 = cc[i - 1 + (5 * k + 4) * ido]
						+ cc[ic - 1 + (5 * k + 3) * ido];
	 ...
	 * @param offset the offset
	 */
	void radfg(int ido, int ip, int l1, int idl1, double cc[], double c1[],
			double c2[], double ch[], double ch2[], final double wtable[],
			int offset) {
		...
		if (ido != 1) {
			...
			if (nbd <= l1) {
				...
				for (j = 1; j < ip; j++) {
					...
					for (i = 2; i < ido; i += 2) {
						idij += 2;
						for (k = 0; k < l1; k++) {
							ch[i - 1 + (k + j * l1) * ido] = wtable[idij - 1
									+ iw1]
									* c1[i - 1 + (k + j * l1) * ido]
									+ wtable[idij + iw1]
									* c1[i + (k + j * l1) * ido];
							ch[i + (k + j * l1) * ido] = wtable[idij - 1 + iw1]
									* c1[i + (k + j * l1) * ido]
									- wtable[idij + iw1]
									* c1[i - 1 + (k + j * l1) * ido];
						}
			...
			if (nbd >= l1) {
				for (j = 1; j < ipph; j++) {
					...
					for (k = 0; k < l1; k++) {
						for (i = 2; i < ido; i += 2) {
							c1[i - 1 + (k + j * l1) * ido] = ch[i - 1
									+ (k + j * l1) * ido]
									+ ch[i - 1 + (k + jc * l1) * ido];
							c1[i - 1 + (k + jc * l1) * ido] = ch[i
									+ (k + j * l1) * ido]
									- ch[i + (k + jc * l1) * ido];
							c1[i + (k + j * l1) * ido] = ch[i + (k + j * l1)
									* ido]
									+ ch[i + (k + jc * l1) * ido];
							c1[i + (k + jc * l1) * ido] = ch[i - 1
									+ (k + jc * l1) * ido]
									- ch[i - 1 + (k + j * l1) * ido];
						}
					}
		...
		if (nbd >= l1) {
			for (j = 1; j < ipph; j++) {
				...
				for (k = 0; k < l1; k++) {
					for (i = 2; i < ido; i += 2) {
						ic = ido - i;
						cc[i - 1 + (j2 + k * ip) * ido] = ch[i - 1
								+ (k + j * l1) * ido]
								+ ch[i - 1 + (k + jc * l1) * ido];
						cc[ic - 1 + (j2 - 1 + k * ip) * ido] = ch[i - 1
								+ (k + j * l1) * ido]
								- ch[i - 1 + (k + jc * l1) * ido];
						cc[i + (j2 + k * ip) * ido] = ch[i + (k + j * l1) * ido]
								+ ch[i + (k + jc * l1) * ido];
						cc[ic + (j2 - 1 + k * ip) * ido] = ch[i + (k + jc * l1)
								* ido]
								- ch[i + (k + j * l1) * ido];
					}
				}
	...
	void radbg(int ido, int ip, int l1, int idl1, double cc[], double c1[],
		...
		for (j = 1; j < ipph; j++) {
			...
			j2 = 2 * j;
			for (k = 0; k < l1; k++) {
				ch[(k + j * l1) * ido] = cc[ido - 1 + (j2 - 1 + k * ip) * ido]
						+ cc[ido - 1 + (j2 - 1 + k * ip) * ido];
				ch[(k + jc * l1) * ido] = cc[(j2 + k * ip) * ido]
						+ cc[(j2 + k * ip) * ido];
			}
		}
			...
			if (nbd >= l1) {
				for (j = 1; j < ipph; j++) {
					...
					for (k = 0; k < l1; k++) {
						for (i = 2; i < ido; i += 2) {
							ic = ido - i;
							ch[i - 1 + (k + j * l1) * ido] = cc[i - 1
									+ (2 * j + k * ip) * ido]
									+ cc[ic - 1 + (2 * j - 1 + k * ip) * ido];
							ch[i - 1 + (k + jc * l1) * ido] = cc[i - 1
									+ (2 * j + k * ip) * ido]
									- cc[ic - 1 + (2 * j - 1 + k * ip) * ido];
							ch[i + (k + j * l1) * ido] = cc[i
									+ (2 * j + k * ip) * ido]
									- cc[ic + (2 * j - 1 + k * ip) * ido];
							ch[i + (k + jc * l1) * ido] = cc[i
									+ (2 * j + k * ip) * ido]
									+ cc[ic + (2 * j - 1 + k * ip) * ido];
						}
					}
		...
		if (nbd >= l1) {
			for (j = 1; j < ipph; j++) {
				...
				for (k = 0; k < l1; k++) {
					for (i = 2; i < ido; i += 2) {
						ch[i - 1 + (k + j * l1) * ido] = c1[i - 1
								+ (k + j * l1) * ido]
								- c1[i + (k + jc * l1) * ido];
						ch[i - 1 + (k + jc * l1) * ido] = c1[i - 1
								+ (k + j * l1) * ido]
								+ c1[i + (k + jc * l1) * ido];
						ch[i + (k + j * l1) * ido] = c1[i + (k + j * l1) * ido]
								+ c1[i - 1 + (k + jc * l1) * ido];
						ch[i + (k + jc * l1) * ido] = c1[i + (k + j * l1) * ido]
								- c1[i - 1 + (k + jc * l1) * ido];
					}
				}
			}
		...
		if (nbd <= l1) {
			...
			for (j = 1; j < ip; j++) {
				...
				for (i = 2; i < ido; i += 2) {
					idij += 2;
					for (k = 0; k < l1; k++) {
						c1[i - 1 + (k + j * l1) * ido] = wtable[idij - 1 + iw1]
								* ch[i - 1 + (k + j * l1) * ido]
								- wtable[idij + iw1]
								* ch[i + (k + j * l1) * ido];
						c1[i + (k + j * l1) * ido] = wtable[idij - 1 + iw1]
								* ch[i + (k + j * l1) * ido]
								+ wtable[idij + iw1]
								* ch[i - 1 + (k + j * l1) * ido];
					}
		...
		factorize_loop: while (true) {
			...
			do {
				...
				if (ntry == 2 && nf != 1) {
					for (i = 2; i <= nf; i++) {
						ib = nf - i + 2;
						wtable[ib + 1 + 2 * n + offset] = wtable[ib + 2 * n
								+ offset];
					}
