package com.tum.ident.fftpack;



/**
 * The Class ComplexDoubleFFT_Mixed.
 *
 * @author Baoshe Zhang
...
class ComplexDoubleFFT_Mixed {
	 ...
	 * @param cc the cc
	 * @param ch the ch
	 * @param wtable the wtable
	 * @param offset the offset
	 * @param isign the isign
	 */
	void passf2(int ido, int l1, final double cc[], double ch[],
		...
		if (ido <= 2) {
			...
			for (k = 0; k < l1; k++) {
				for (i = 0; i < ido - 1; i += 2) {
					...
					ch[ah + l1 * ido + 1] = wtable[i + iw1] * ti2 + isign
							* wtable[i + 1 + iw1] * tr2;
					ch[ah + l1 * ido] = wtable[i + iw1] * tr2 - isign
							* wtable[i + 1 + iw1] * ti2;
				}
			}
		}
	 ...
	 * @param isign the isign
	 */
	void passf3(int ido, int l1, final double cc[], double ch[],
			final double wtable[], int offset, int isign) {
		...
		if (ido == 2) {
			...
			for (k = 1; k <= l1; k++) {
				for (i = 0; i < ido - 1; i += 2) {
					...
					di2 = ci2 + cr3;
					di3 = ci2 - cr3;
					ch[ah + l1 * ido + 1] = wtable[i + iw1] * di2 + isign
							* wtable[i + 1 + iw1] * dr2;
					ch[ah + l1 * ido] = wtable[i + iw1] * dr2 - isign
							* wtable[i + 1 + iw1] * di2;
					ch[ah + 2 * l1 * ido + 1] = wtable[i + iw2] * di3 + isign
							* wtable[i + 1 + iw2] * dr3;
					ch[ah + 2 * l1 * ido] = wtable[i + iw2] * dr3 - isign
							* wtable[i + 1 + iw2] * di3;
				}
			}
		}
	 ...
	 * @param isign the isign
	 */
	void passf4(int ido, int l1, final double cc[], double ch[],
			final double wtable[], int offset, int isign) {
		int i, k, ac, ah;
		...
		if (ido == 2) {
			...
			for (k = 0; k < l1; k++) {
				for (i = 0; i < ido - 1; i += 2) {
					...
					ci2 = ti1 + isign * ti4;
					ci4 = ti1 - isign * ti4;
					ch[ah + l1 * ido] = wtable[i + iw1] * cr2 - isign
							* wtable[i + 1 + iw1] * ci2;
					ch[ah + l1 * ido + 1] = wtable[i + iw1] * ci2 + isign
							* wtable[i + 1 + iw1] * cr2;
					ch[ah + 2 * l1 * ido] = wtable[i + iw2] * cr3 - isign
							* wtable[i + 1 + iw2] * ci3;
					ch[ah + 2 * l1 * ido + 1] = wtable[i + iw2] * ci3 + isign
							* wtable[i + 1 + iw2] * cr3;
					ch[ah + 3 * l1 * ido] = wtable[i + iw3] * cr4 - isign
							* wtable[i + 1 + iw3] * ci4;
					ch[ah + 3 * l1 * ido + 1] = wtable[i + iw3] * ci4 + isign
							* wtable[i + 1 + iw3] * cr4;
				}
			}
		}
	 ...
	 * @param isign the isign
	 */
	void passf5(int ido, int l1, final double cc[], double ch[],
			final double wtable[], int offset, int isign)
		...
		if (ido == 2) {
			...
			for (k = 1; k <= l1; k++) {
				for (i = 0; i < ido - 1; i += 2) {
					...
					ch[ah + 2 * l1 * ido + 1] = wtable[i + iw2] * di3 + isign
							* wtable[i + 1 + iw2] * dr3;
					ch[ah + 3 * l1 * ido] = wtable[i + iw3] * dr4 - isign
							* wtable[i + 1 + iw3] * di4;
					ch[ah + 3 * l1 * ido + 1] = wtable[i + iw3] * di4 + isign
							* wtable[i + 1 + iw3] * dr4;
					ch[ah + 4 * l1 * ido] = wtable[i + iw4] * dr5 - isign
							* wtable[i + 1 + iw4] * di5;
					ch[ah + 4 * l1 * ido + 1] = wtable[i + iw4] * di5 + isign
							* wtable[i + 1 + iw4] * dr5;
				}
			}
		}
	...
	void passfg(int nac[], int ido, int ip, int l1, int idl1,
			final double cc[], double c1[], double c2[], double ch[],
			double ch2[], final double wtable[], int offset, int isign) {
		...
		if (ido >= l1) {
			for (j = 1; j < ipph; j++) {
				...
				for (k = 0; k < l1; k++) {
					for (i = 0; i < ido; i++) {
						ch[i + (k + j * l1) * ido] = cc[i + (j + k * ip) * ido]
								+ cc[i + (jc + k * ip) * ido];
						ch[i + (k + jc * l1) * ido] = cc[i + (j + k * ip) * ido]
								- cc[i + (jc + k * ip) * ido];
					}
				}
			}
		...
		for (l = 1; l < ipph; l++) {
			...
			idl += ido;
			for (ik = 0; ik < idl1; ik++) {
				c2[ik + l * idl1] = ch2[ik] + wtable[idl - 2 + iw1]
						* ch2[ik + idl1];
				c2[ik + lc * idl1] = isign * wtable[idl - 1 + iw1]
						* ch2[ik + (ip - 1) * idl1];
			}
			idlj = idl;
			inc += ido;
		...
		for (j = 1; j < ipph; j++) {
			...
			for (ik = 1; ik < idl1; ik += 2) {
				ch2[ik - 1 + j * idl1] = c2[ik - 1 + j * idl1]
						- c2[ik + jc * idl1];
				ch2[ik - 1 + jc * idl1] = c2[ik - 1 + j * idl1]
						+ c2[ik + jc * idl1];
				ch2[ik + j * idl1] = c2[ik + j * idl1] + c2[ik - 1 + jc * idl1];
				ch2[ik + jc * idl1] = c2[ik + j * idl1]
						- c2[ik - 1 + jc * idl1];
			}
		}
		nac[0] = 1;
		...
		if (idot <= l1) {
			...
			for (j = 1; j < ip; j++) {
				...
				for (i = 3; i < ido; i += 2) {
					idij += 2;
					for (k = 0; k < l1; k++) {
						c1[i - 1 + (k + j * l1) * ido] = wtable[idij - 2 + iw1]
								* ch[i - 1 + (k + j * l1) * ido] - isign
								* wtable[idij - 1 + iw1]
								* ch[i + (k + j * l1) * ido];
						c1[i + (k + j * l1) * ido] = wtable[idij - 2 + iw1]
								* ch[i + (k + j * l1) * ido] + isign
								* wtable[idij - 1 + iw1]
								* ch[i - 1 + (k + j * l1) * ido];
					}
				}
	...
	void cfftf1(int n, double c[], final double wtable[], int isign) {
		...
		for (k1 = 2; k1 <= nf + 1; k1++) {
			...
			if (ip == 4) {
				...
				if (na == 0) {
					passfg(nac, idot, ip, l1, idl1, c, c, c, ch, ch, wtable,
							iw, isign);
				} else {
					passfg(nac, idot, ip, l1, idl1, ch, ch, ch, c, c, wtable,
							iw, isign);
				}
				if (nac[0] != 0)
					na = 1 - na;
